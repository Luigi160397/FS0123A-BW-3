...
const AddComment = ({ post }) => {
    ...
    return (
        <Card className="mt-3 mb-3 text-white bg-dark">
            <Card.Body className="d-flex align-items-center gap-2 px-0">
                ...
                <Form className="w-100" onSubmit={sendComment}>
                    <Form.Control className="bg-dark text-light rounded-pill" type="text" placeholder="Scrivi un commento.." value={comment.comment} onChange={e => setComment({ ...comment, comment: e.target.value })} />
                </Form>
            </Card.Body>
        </Card>
    );
